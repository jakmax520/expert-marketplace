---
name: cdgpt-collection-skill
description: "腾讯健康组学平台(Omics) - CD-GPT Collection (Nextflow) 公共应用合集专用运行助手。CD-GPT是一种生成式生物基础大模型，旨在捕捉生物系统中复杂的全系统分子互作关系。通过对DNA、RNA及蛋白质序列等全分子层级数据进行预训练，能高效处理一系列下游任务，包括单分子分析与多分子联合分析仅服务于本合集子应用的导入与运行；其他公共应用、本地 WDL、COS 上的 NF、项目内已有应用请改用 omics-task-skill。 触发词：CD-GPT Collection (Nextflow), cd-gpt collection (nextflow), cd-gpt-collection-(nextflow), cd-gpt_collection_(nextflow), cd-gptcollection(nextflow), AI 模型, ai 模型, 单细胞与多组学基础模型, 是一种生成式生物基础大模型, 生成式生物基础大模型, CD-GPT, 的全系统分子互作关系, 互作关系, DNA, RNA, 包括单分子分析与多分子联合分析, 分析与多分子联合分析, 跑 CD-GPT Collection (Nextflow), run cd-gpt collection (nextflow), 合集"
platform: 腾讯健康组学平台(Omics)
tags: [行业专业, 生信分析, 生物医药, omics, 腾讯健康组学平台, public-app, collection]
version: 1.0.0
---

# CD-GPT Collection (Nextflow) Skill (v1.0.0 · 单合集收窄版)

> **平台归属**：腾讯健康组学平台(Omics) 公共应用合集专用 SKILL。
> 本 SKILL 是 `omics-task-skill` 的**单合集收窄版**：仅服务于 `CD-GPT Collection (Nextflow)` (NEXTFLOW) 这一个公共应用合集。
> 所有命令拼接走 `scripts/omics_cli.py`，统一参数与输出格式。
> **能力范围严格 = `omics-platform-cli` 7 命令 ∩ 仅运行 CD-GPT Collection (Nextflow) 合集下的子应用**；任何越界都视为越权。
...
| **PublicAppId**        | `publicapp-nextflow-cd-gpt-collection` |
| **应用类型**           | `NEXTFLOW`                     |
| **分组类型**           | `APP_COLLECTION`（合集，含多个子应用） |
| **标签**               | `行业专业, 生信分析, 生物医药, omics, 腾讯健康组学平台, public-app, collection`                     |

**应用简介**：CD-GPT是一种生成式生物基础大模型，旨在捕捉生物系统中复杂的全系统分子互作关系。通过对DNA、RNA及蛋白质序列等全分子层级数据进行预训练，能高效处理一系列下游任务，包括单分子分析与多分子联合分析。
...
1. **严禁运行除 CD-GPT Collection (Nextflow) 之外的任何应用**：
   ...
   - **唯一允许**的 run 形态：`omics run --public-app <CD-GPT Collection (Nextflow) 合集展开后的子应用 AppId> ...`
...
> ⚠️ `--parent-app` 值**固定**为 CD-GPT Collection (Nextflow) 的 AppId，不接受替换。

### 3.1 转述给用户

> CD-GPT Collection (Nextflow) 包含以下子应用，请挑一个具体的子流程：
>
...
即将运行任务，请确认：
  ┌──────────────────────────────────────────────────┐
  │ 形态        : 公共应用（form B，自动模板）        │
  │ 来源合集    : CD-GPT Collection (Nextflow)                   │
  │ 子应用      : <selectedAppName>                   │
  │ AppId       : <selectedAppId>                     │
...
| `公共应用 X 是一个合集` / `AppId 不存在`  | 核对 `--public-app` 是否传成了合集 AppId？合集 AppId（`5e3b30ee-85a2-4ad7-80c5-fa2f0ecb0d0c`）不可作为 run 的 `--public-app` |
...
3. ✅ 唯一合法响应：「本 SKILL 只能运行 CD-GPT Collection (Nextflow) 下的子应用。如需运行其他应用（含项目已有应用），请使用 `omics-task-skill`。」
...
# cli.execute(cli.build_run(public_app="5e3b30ee-85a2-4ad7-80c5-fa2f0ecb0d0c", ...)) # 合集本身
