# CLI / SKILL 边界契约（scBERT 单应用收窄版 v1.0.0)
...
| 6   | `omics run`                       | —                                      | 写远端任务   | ✅ **必须先二次确认；`--public-app` `--public-app` 固定为 `0de1b4ae-1543-4882-8bae-b5ee87968bc5`**                    |
| 7   | `omics status`                    | —                                      | read           | ✅                                                                                                 |
| 8   | `omics debug`                     | `<rgId>` / `--run` / `--run + --job`   | read           | ✅                                                                                                 |

> 应用 AppId（锁定）：`0de1b4ae-1543-4882-8bae-b5ee87968bc5`
...
| 反例                                                                                          | 违反原则                                                            |
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| SKILL 调 `omics run --wdl <path>`（form A）                                                   | 越界——本 SKILL 只能跑 scBERT                              |
...
用户："跑 scBERT" / "做 XX 分析"
        │
        ▼
SKILL 拼: run --public-app 0de1b4ae-1543-4882-8bae-b5ee87968bc5
        │
        ▼
输出参数摘要表 + 完整命令 → 询问 y/N
        │
...
2. 摘要表必须列出：应用名 + AppId / 项目 / 环境 / NF 版本 / 导入命名
