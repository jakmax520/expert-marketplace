---
name: scbert-skill
description: "腾讯健康组学平台(Omics) - scBERT 公共应用专用运行助手。scBERT是一个单细胞预训练模型，基于BERT范式，高效提取临床单细胞转录组的特征，可用于细胞类型注释，新类发现，marker基因检测等肿瘤微环境分析场景仅服务于本应用的导入与运行；其他公共应用、本地 WDL、COS 上的 NF、项目内已有应用请改用 omics-task-skill。 触发词：scBERT, scbert, AI 模型, ai 模型, 单细胞与多组学基础模型, BERT, 等肿瘤微环境分析场景, 分析场景, marker, 跑 scBERT, run scbert, 应用"
...
| **PublicAppId**    | `publicapp-nextflow-scbert` |
| **应用类型**       | `NEXTFLOW`                     |
| **分组类型**       | 独立应用                               |
| **标签**           | `行业专业, 生信分析, 生物医药, omics, 腾讯健康组学平台, public-app`                     |

**应用简介**：scBERT是一个单细胞预训练模型，基于BERT范式，高效提取临床单细胞转录组的特征，可用于细胞类型注释，新类发现，marker基因检测等肿瘤微环境分析场景。
...
| **同名检查（后执行）** | 检查是否有与 `scBERT`（或用户指定的 `--public-app-name`）同名的已有应用 |
...
```bash
omics run --public-app 0de1b4ae-1543-4882-8bae-b5ee87968bc5 \\
...
即将运行任务，请确认：
  ...
  │ 项目/环境  : ← config                       │
  └────────────────────────────────────────────┘
完整命令: omics run --public-app 0de1b4ae-1543-4882-8bae-b5ee87968bc5 ...
...
2. 不要替用户做症状判断 + 自动改代码
3. stderr 整段贴出
4. 修复后统一走 `omics run --public-app 0de1b4ae-1543-4882-8bae-b5ee87968bc5` 重发
