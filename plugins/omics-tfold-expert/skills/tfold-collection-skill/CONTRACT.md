# CLI / SKILL 边界契约（tFold Collection (Nextflow) 单合集收窄版 v1.0.0)

> 本文档定义 omics-platform-cli 与 `tfold-collection-skill` 之间不可违反的能力边界。
> CR 与日常巡检按本文档执行。
...
| 反例                                                                                          | 违反原则                                                            |
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| SKILL 调 `omics run --wdl <path>`（form A）                                                   | 越界——本 SKILL 只能跑 tFold Collection (Nextflow)                              |
| SKILL 调 `omics run --app <ApplicationId>`（form C）                                          | 越界——引导用户去 `omics-task-skill`                                  |
| SKILL 调 `omics run --public-app 807fc9ae-6197-43c0-a7a8-993c09ec1ee2 ...`（合集本身） | 合集不可直接 run                                                               |
...
```
用户："跑 tFold Collection (Nextflow)" / "做 XX 分析"
        │
        ▼
SKILL 拼: list public-apps --parent-app 807fc9ae-6197-43c0-a7a8-993c09ec1ee2 -o json
        │
        ▼
