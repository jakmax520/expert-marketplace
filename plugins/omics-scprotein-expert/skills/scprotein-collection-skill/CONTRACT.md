# CLI / SKILL 边界契约（scPROTEIN Collection (Nextflow) 单合集收窄版 v1.0.0)

> 本文档定义 omics-platform-cli 与 `scprotein-collection-skill` 之间不可违反的能力边界。
> CR 与日常巡检按本文档执行。
...
scprotein-collection-skill 通过 wrapper（`scripts/omics_cli.py`）调用 CLI；wrapper 在 argparse 层物理上注册 7 条一级命令，
但本 SKILL **只允许**调用以下子集：
...
| 4   | `omics list public-apps`          | `--parent-app <CollectionAppId>` | read           | ✅ **必须带 `--parent-app 5c63718b-d24f-4a9f-9838-4177185f414a`**；其它形态禁用                                                                    |
...
| 反例                                                                                          | 违反原则                                                            |
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| SKILL 调 `omics run --wdl <path>`（form A）                                                   | 越界——本 SKILL 只能跑 scPROTEIN Collection (Nextflow)                              |
| SKILL 调 `omics run --app <ApplicationId>`（form C）                                          | 越界——引导用户去 `omics-task-skill`                                  |
| SKILL 调 `omics run --public-app 5c63718b-d24f-4a9f-9838-4177185f414a ...`（合集本身） | 合集不可直接 run                                                               |
...
```
用户："跑 scPROTEIN Collection (Nextflow)" / "做 XX 分析"
        │
        ▼
SKILL 拼: list public-apps --parent-app 5c63718b-d24f-4a9f-9838-4177185f414a -o json
        │
        ▼
...
| SKILL 是否引入 HTTP 客户端                                                                    | `grep -nE "import (requests|http|httpx|urllib)" skills/scprotein-collection-skill/` 应为空                                                                       |
